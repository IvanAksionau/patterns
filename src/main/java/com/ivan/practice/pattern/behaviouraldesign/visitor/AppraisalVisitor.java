package com.ivan.practice.pattern.behaviouraldesign.visitor;
...
public class AppraisalVisitor implements Visitor {
    ...
    private int getTeamAverage(Employee emp) {
        return (int) Math.round(emp.getDirectReports()
                .stream()
                .mapToDouble(Employee::getPerformanceRating).average().getAsDouble());
    }
